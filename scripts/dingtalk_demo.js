...
(function () {
    ...
    var BACKEND_FIELDS = [
        ...
        '箱数', '毛重', '净重', '毛重（千克）', '净重（千克）', '系统号码', '发货单号'
    ];
    ...
    for (var oi = 0; oi < orderEntries.length; oi++) {
        ...
        for (var r = 1; r < allValues.length; r++) {
            ...
            // 文件名发货单号来源：优先系统号码，统一归一到“发货单号”。
            var normalizedShipmentNo = firstNonEmptyField(obj, ['系统号码', '发货单号', '送货单号', '出货单号']);
            if (normalizedShipmentNo) {
                if (!obj['发货单号']) {
                    obj['发货单号'] = normalizedShipmentNo;
                }
                if (!obj['系统号码']) {
                    obj['系统号码'] = normalizedShipmentNo;
                }
            }
